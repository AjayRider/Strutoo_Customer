...
class SSCustomBottomNavigation : FrameLayout {
    ...
    var cbnMenuItems: Array<Model> = arrayOf()
    ...
    fun setupWithNavController(navController: NavController) {
        ...
        navController.addOnDestinationChangedListener { _, destination, _ ->
            Log.e("TAG", "setupWithNavController: ${destination.label}")
            cbnMenuItems.map { it.destinationId }.indexOf(destination.id).let { index ->
                if (index != -1) {
    ...
    // source code referenced from the actual JetPack Navigation Component
    // refer to the original source code
    fun matchDestination(destination: NavDestination, @IdRes destinationId: Int): Boolean {
